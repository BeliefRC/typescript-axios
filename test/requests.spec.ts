import axios, { AxiosResponse, IAxiosError } from '../src/index'
...
describe('requests', () => {
  ...
  test('should reject on network errors', () => {
    ...
    const rejectSpy = jest.fn((e: IAxiosError) => {
      return e
    })
    ...
    function next(reason: AxiosResponse | IAxiosError) {
  ...
  test('should reject when validateStatus returns false', () => {
    ...
    function next(reason: IAxiosError | AxiosResponse) {
      expect(resolveSpy).not.toHaveBeenCalled()
      expect(rejectSpy).toHaveBeenCalled()
      expect(reason instanceof Error).toBeTruthy()
      expect((reason as IAxiosError).message).toBe('Request failed with status code 500')
      expect((reason as IAxiosError).response!.status).toBe(500)
    }
  })
  ...
  test('should resolve when validateStatus returns true', () => {
    ...
    function next(res: AxiosResponse | IAxiosError) {
